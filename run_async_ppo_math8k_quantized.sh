#!/bin/bash
# Async PPO training script with TIS and Flash-RL quantization on 8k math dataset
# This script enables async rollout pipeline mode where rollout for batch t+1 runs
# concurrently with learner updates for batch t, improving GPU utilization
# Usage: bash run_async_ppo_math8k_quantized.sh [RUN_NAME] [QUANTIZATION_TYPE] [FP32_LM_HEAD]
...
if [ -z "$RUN_NAME" ]; then
    RUN_NAME="async_ppo_math8k_${QUANTIZATION_TYPE}_Qwen2.5-32B_$(date +%Y%m%d-%H%M%S)"
fi
...
echo "=========================================="
echo "Flash-RL Async PPO Training with TIS"
echo "=========================================="
...
# Run Async PPO training with TIS and quantization
# Key differences from sync PPO:
# - actor_rollout_ref.rollout.mode=async: enables async rollout server
